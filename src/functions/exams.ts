...
import { KeyPair, PupilEnvelope, VulcanApiResponse, ExamEnvelope } from '../types';
...
export default async (keyPair:KeyPair, restUrl: string, pupil: PupilEnvelope, dateFrom: Date, dateTo: Date) => {
	...
	const url = `${strings.BASE_URL}/${tenant}/${pupil.Unit.Symbol}/api/mobile/exam/byPupil?unitId=${pupil.Unit.Id}&pupilId=${pupil.Pupil.Id}&dateFrom=${moment(dateFrom).format('YYYY-MM-DD')}&dateTo=${moment(dateTo).format('YYYY-MM-DD')}&pageSize=100`;
	const date = new Date();
	...
	// @ts-ignore
	const data:VulcanApiResponse<Array<ExamEnvelope>> = await aab.json();
	handleErrors(data);
	return data as VulcanApiResponse<Array<ExamEnvelope>>
}